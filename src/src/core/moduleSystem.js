 ...
 * Each module has its own lifecycle and can declare dependencies and version requirements.
 * The main flow is register -> init -> start -> stop.
 */
...
// id -> { definition, state, context } for every registered module
...
// Dependencies are checked before init so a module cannot start before its dependencies are ready.
...
// init = prepare the module; it is not considered active until start() succeeds.
...
// start = activate the module after init succeeds
function start(id) {
    const record = modules.get(id);
    if (!record) return false;
...
// bootAll intentionally initializes every module first, then starts them so modules can see initialized dependencies.
