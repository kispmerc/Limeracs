/**
 * In-memory cache with tick-based TTL
 * This cache is not persistent storage: data is lost when the world/pack restarts.
 */

import { system } from '@minecraft/server';

// key -> { value, expiresAt }; memory-only cache
const entries = new Map();
let sweepHandle = null;

// Sweep every 200 ticks to remove expired entries even when nobody calls get().
